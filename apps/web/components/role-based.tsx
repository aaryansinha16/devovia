 ...
 */
export function AdminOnly({ children, fallback = null }: Omit<RoleBasedComponentProps, 'allowedRoles'>) {
  return (
 ...
 */
export function ModeratorOrAdmin({ children, fallback = null }: Omit<RoleBasedComponentProps, 'allowedRoles'>) {
  return (
    <RoleBasedComponent allowedRoles={['ADMIN', 'MODERATOR']} fallback={fallback}>
      {children}
    </RoleBasedComponent>
  );
}
