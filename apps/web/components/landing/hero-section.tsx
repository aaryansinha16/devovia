...
const HeroSection: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const calculatedOpacity = Math.max(0, Math.min(1, 1 - scrollY / fadeOutDistance));
  ...
  return (
    ...
    >
      ...
      <div id="hero-background-elements" className="absolute inset-0 -z-10 overflow-hidden">
      ...
      <div id="hero-content-wrapper" className="container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 hero-content"> 
        <AnimatedElement animationClassName="animate-[fadeInUp_0.8s_ease-out_forwards]" as="h1" id="hero-main-heading" className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight hero-title">
          <span className="block text-slate-800 dark:text-slate-100">Devovia: Your Developer</span>
          <span className="block text-sky-500 dark:text-sky-400">Command Center</span>
        </AnimatedElement>

        <AnimatedElement animationClassName="animate-[fadeInUp_0.8s_ease-out_forwards]" delay="[animation-delay:200ms]" as="p" id="hero-subtitle" className="mt-6 max-w-lg mx-auto text-lg md:text-xl text-slate-600 dark:text-slate-300 sm:max-w-2xl hero-description">
          Streamline your workflow with powerful tools for code snippets, project templates, CI/CD pipelines, and more. Build, share, and innovate—faster.
        </AnimatedElement>

        <AnimatedElement animationClassName="animate-[fadeInUp_0.8s_ease-out_forwards]" delay="[animation-delay:400ms]" id="hero-cta-buttons" className="mt-10 flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6 hero-actions">
          <Button href="#early-access" variant="primary" size="lg" rightIcon={<ArrowRightIcon className="w-5 h-5" />} id="hero-cta-primary">
            Get Early Access
          </Button>
          <Button href="#features" variant="outline" size="lg" id="hero-cta-secondary">
            Explore Features
          </Button>
        </AnimatedElement>
      </div>
